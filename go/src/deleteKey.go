...
func main() {
	...
	// Convert request to JSON
	jsonData, err := json.Marshal(deleteKeyReq)
	if err != nil {
		log.Fatal("Error creating JSON request:", err)
	}

	// Create HTTP request
	req, err := http.NewRequest("POST", BASE_URL+"/admin/keys/delete-key", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		log.Fatalf("Failed to delete key: %s", resp.Status)
	}
