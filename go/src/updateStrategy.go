...
func main() {
	...
	updateStrategyReq := UpdateStrategyRequest{
		Code:     "claude-proxy-1",
		Type:     "proxy",
	...
	// Convert request to JSON
	jsonData, err := json.Marshal(updateStrategyReq)
	if err != nil {
		log.Fatal("Error creating JSON request:", err)
	}

	// Create HTTP request
	req, err := http.NewRequest("POST", BASE_URL+"/admin/strategies/update-strategy", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	if resp.StatusCode != http.StatusOK {
		...
		log.Fatalf("Failed to update strategy: %s", resp.Status)
	}
