...
async fn main() -> Result<()> {
    ...
    // Delete the key
    let delete_key_req = DeleteKeyRequest {
        key_id: "claude-key-1".to_string(),
    };

    let client = reqwest::Client::new();
    let response = client
        .post(format!("{}/admin/keys/delete-key", BASE_URL))
    ...
    if !response.status().is_success() {
        ...
        if status == reqwest::StatusCode::NOT_FOUND {
            println!("\n{}", "⚠️  Key not found - it may have already been deleted".yellow());
            return Ok(());
        }
        
        let error_text = response.text().await?;
        println!("\n{} {} {}", "Server response:".red(), status, error_text);
        anyhow::bail!("Failed to delete key: {}", status);
    }

    println!("\n{}", "✅ Key deleted successfully".green());
