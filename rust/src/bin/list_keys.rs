...
struct KeyInfo {
    ...
    key_id: String,
    provider: String,
    // Add other fields as needed, but note that sensitive data is excluded
}
...
async fn main() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let response = client
        .get(format!("{}/admin/keys/list", BASE_URL))
    ...
    if !response.status().is_success() {
        ...
        anyhow::bail!("Failed to list keys: {}", status);
    }

    let result: ListKeysResponse = response.json().await?;
    println!("\n{}", "✅ Keys listed successfully".green());
    println!("{:#?}", result.keys);
    
    println!("\n{}", "Note that neither the API key values nor the encryption fragments are included in the response.".cyan());
