...
async fn main() -> Result<()> {
    ...
    // Add a new strategy
    let add_strategy_req = AddStrategyRequest {
    ...
    let client = reqwest::Client::new();
    let response = client
        .post(format!("{}/admin/strategies/add-strategy", BASE_URL))
        .header("Content-Type", "application/json")
        .header("Authorization", format!("Bearer {}", token))
        .json(&add_strategy_req)
        .send()
        .await?;
    ...
    if !response.status().is_success() {
        ...
        anyhow::bail!("Failed to add strategy: {}", status);
    }

    println!("\n{}", "✅ Strategy added successfully".green());
